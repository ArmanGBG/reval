...
async function main() {
  ...
  const taskSubjects = await Promise.all(
    [
      ['ریاضی', '#3EB489'],
      ['فیزیک', '#F59E0B'],
      ['شیمی', '#EF4444'],
      ...
      prisma.subject.upsert({
        where: { normalizedName: normalizePersianText(name) },
        update: {},
        create: { name, normalizedName: normalizePersianText(name), color, isKonkur: name !== 'ادبیات' },
      }),
    ),
  );
  ...
  const tasksData = [
    ...
    { studentId: student4.id, subject: 'زیست‌شناسی', subjectColor: '#8B5CF6', topic: 'سلول', fieldType: 'کنکور', activityTypes: '["مرور","تست سنجشی"]', targetTimeMinutes: 90, targetTestCount: 40, createdBy: 'advisor', createdById: advisor2.id, order: 1 },
  ...
  for (const t of tasksData) {
    ...
    if (!subject) throw new Error(`Subject not seeded: ${t.subject}`);
