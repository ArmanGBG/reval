...
- [x] مرحله ۳: طراحی و اجرای مدل وضعیت تسک
- [x] مرحله ۴: migration و پاک‌سازی داده‌های قبلی
- [x] مرحله ۵: اصلاح API و authorization تسک
- [x] مرحله ۶: یکسان‌سازی ساخت دستی و ساخت توسط مشاور
- [x] مرحله ۷: اصلاح برنامه هفتگی و Draftهای چندروزه
- [x] مرحله ۸: تفکیک Draft و Incomplete در UI
- [x] مرحله ۹: ساخت سرویس گزارش مشترک
- [x] مرحله ۱۰: اتصال گزارش مشاور به داده واقعی
- [x] مرحله ۱۱: حذف تحلیل‌های ساختگی و nullable کردن داده‌های غایب
- [x] مرحله ۱۲: اصلاح گزارش فصل‌محور و Weekly Goal
...
## ۲۴. استراتژی دیباگ رگرسیون تعامل و گزارش

برای رفع باگ‌های lifecycle و گزارش، هر عملیات باید از UI تا دیتابیس بررسی شود:

1. `COMPLETE`: ثبت هم‌زمان `status=COMPLETED`، `completed=true` و actualهای معتبر.
2. `REDO`: مجاز برای دانش‌آموز روی تسک مشاور، با `status=PENDING` و `completed=null`.
3. `MOVE_DATE`: Draft فقط تاریخش تغییر کند؛ تسک کامل به روز جدید با `status=INCOMPLETE` منتقل شود.
4. `INCOMPLETE`: فقط تسک دارای جزئیات اجرایی؛ Draft هرگز به Incomplete تبدیل نشود.
5. `DELETE`: دانش‌آموز فقط تسک خودش را حذف کند؛ تسک مشاور حذف‌نشدنی باشد.
6. `AUTO_ROLLOVER`: تسک Pending بدون اقدام پس از پایان روز به روز بعد و `INCOMPLETE` منتقل شود.
7. گزارش روزانه، هفتگی، ماهانه و بازه دلخواه باید از `status` و actual استفاده کند؛ target فقط برای برنامه‌ریزی نمایش داده شود.
8. گزارش دانش‌آموز و مشاور باید با یک fixture واقعی یکسان مقایسه شوند.
