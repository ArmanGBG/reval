...
export async function POST(request: NextRequest) {
  ...
  const firstName = typeof body.firstName === 'string' ? body.firstName.trim() : (typeof body.name === 'string' ? body.name.trim().split(' ')[0] : '');
  const lastName = typeof body.lastName === 'string' && body.lastName.trim() ? body.lastName.trim() : (typeof body.name === 'string' && body.name.trim().includes(' ') ? body.name.trim().split(' ').slice(1).join(' ') : null);
  ...
  const user = await db.user.create({ data: { phone, firstName, lastName, role, grade, major, province, city, instituteId, publicCode: await createPublicCode(role === 'ADVISOR' ? 'ADV' : 'STU'), isActive: true }, select: userSelect });
