...
import { GradeChaptersTree } from './GradeChaptersTree';
...
export function SubjectDetail({ subject: initialSubject, onBack, onChange }: SubjectDetailProps) {
  ...
  const chaptersByGrade = (GRADES as readonly string[]).map((g) => ({
    grade: g,
    chapters: (subject.chapters || []).filter((c) => c.grade === g),
  })).filter((g) => g.chapters.length > 0);

  // Find grade-subject config per grade
  const gradeConfigs: Record<string, GradeSubject | undefined> = {};
  for (const gs of subject.grades || []) {
    gradeConfigs[gs.grade] = gs;
  }
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {tab === 'tree' && (
            <GradeChaptersTree
              subject={subject}
              chaptersByGrade={chaptersByGrade}
