...
import { Send, Mail, Loader2, Inbox, Users, CheckCheck, Crown } from 'lucide-react';
...
export default function SuperAdminMessages() {
  ...
  void user; // user context not directly needed — auth via cookie
  ...
  // ===== Load ALL students (super-admin only) =====
  useEffect(() => {
    let mounted = true;
    ...
    (async () => {
      try {
        ...
        setStudents(
          rows.map((u: { id: string; name: string; phone: string; avatar: string }) => ({
            id: u.id,
            name: u.name,
            phone: u.phone,
            avatar: u.avatar,
  ...
  const handleSend = async () => {
    ...
    // Empty string → null (true broadcast for super-admin)
  ...
  return (
    <div className="space-y-5 md:space-y-6 animate-fade-in-up" dir="rtl">
      {/* ============ Page Header ============ */}
      <header className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 md:w-12 md:h-12 rounded-[14px] bg-[var(--gold-soft)] border border-[var(--border)] flex items-center justify-center shrink-0">
            <Send className="w-5 h-5 md:w-6 md:h-6 text-[var(--gold)]" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg md:text-2xl font-bold text-[var(--foreground)] leading-tight">پیام‌رسانی</h1>
              <span className="inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-[var(--gold-soft)] text-[var(--gold)] border border-[var(--border)]">
                <Crown className="w-3 h-3" />
                GOD
              </span>
            </div>
            <p className="text-xs md:text-sm text-[var(--foreground-muted)] mt-0.5">
              ارسال پیام سراسری یا به دانش‌آموز خاص
            </p>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-5 md:gap-6">
        ...
        >
          ...
          <input
            ...
            disabled={sending}
            maxLength={120}
            placeholder="مثلاً: اطلاعیه نگارش اقلیم کنکور ۱۴۰۴"
            className="w-full bg-[var(--bg-overlay)] border border-[var(--border)] rounded-[10px] px-3 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--foreground-subtle)] focus:outline-none focus:border-[var(--gold)]/50 transition-colors mb-2 disabled:opacity-60"
          />
          ...
          <textarea
            ...
            maxLength={2000}
            rows={6}
            placeholder="متن پیام رو اینجا بنویس..."
            className="w-full bg-[var(--bg-overlay)] border border-[var(--border)] rounded-[10px] px-3 py-2.5 text-sm text-[var(--foreground)] placeholder:text-[var(--foreground-subtle)] focus:outline-none focus:border-[var(--gold)]/50 transition-colors resize-none mb-2 disabled:opacity-60"
          />
          <div className="text-[10px] text-[var(--foreground-subtle)] mb-4 text-left">
        ...
        >
          ...
          {loadingSent ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
              <div className="w-12 h-12 rounded-2xl bg-[var(--gold-soft)] flex items-center justify-center mb-3">
