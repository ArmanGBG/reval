...
export default function AiEntryModal({
  ...
  const handleConfirm = useCallback(async () => {
    if (!user?.grade || !user.major) {
      toast.error('ابتدا پایه و رشته را در پروفایل تکمیل کنید');
      return;
    }
    const subjectsByField = new Map<string, Array<{ id: string; name: string; color: string }>>();
    for (const fieldType of new Set(parsedTasks.map((task) => task.field_type))) {
      const response = await fetch(`/api/subjects/for-task?fieldType=${encodeURIComponent(fieldType)}&grade=${encodeURIComponent(user.grade)}&major=${encodeURIComponent(user.major)}`);
      if (!response.ok) {
        toast.error('دریافت فهرست دروس ناموفق بود');
        return;
      }
      const data = await response.json();
      subjectsByField.set(fieldType, data.subjects || []);
    }
    const unresolved = parsedTasks.find((task) =>
      !subjectsByField.get(task.field_type)?.some((subject) => subject.name === task.subject),
    );
    if (unresolved) {
      toast.error(`درس «${unresolved.subject}» برای این دانش‌آموز تعریف نشده است`);
      return;
    }
    const newTasks: Task[] = parsedTasks.map((pt, i) => {
      const subjectObj = subjectsByField.get(pt.field_type)!.find((subject) => subject.name === pt.subject)!;
      return {
    ...
    try {
      await onConfirm(newTasks);
      toast.success(`${toPersianNum(newTasks.length)} تسک اولیه اضافه شد`);
      handleClose();
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'ثبت تسک‌ها ناموفق بود');
    }
  }, [parsedTasks, selectedDate, existingTaskCount, onConfirm, handleClose, studentId, user]);
