...
import { Check, X, Settings, Trash2, Clock, FileText } from 'lucide-react';
...
// ===== Component =====
export default function TaskCard({ task, index, onComplete, onSkip, onDelete, onSettings }: TaskCardProps) {
    ...
    >
      <div className="flex items-start justify-between gap-3 md:gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {/* Topic */}
          <p className="text-[var(--foreground-muted)] text-xs md:text-sm mb-2.5 line-clamp-2">{task.topic}</p>
          ...
          <div className="flex flex-wrap gap-1.5 mb-2.5">
            {task.activityTypes.map((at) => (
              <span
                key={at}
                className="text-[10px] px-2 py-0.5 rounded-md bg-[rgba(255,255,255,0.04)] text-[var(--foreground-muted)] border border-[var(--border)]"
              >
                {at}
              </span>
            ))}
          ...
          <div className="flex items-center gap-3 text-xs text-[var(--foreground-muted)]">
            <span className="flex items-center gap-1.5">
              ...
              {toPersianNum(task.targetTimeMinutes)} دقیقه
            </span>
            {task.targetTestCount > 0 && (
              <>
                ...
                <span className="flex items-center gap-1.5">
                  ...
                  {toPersianNum(task.targetTestCount)} تست
                </span>
              </>
            )}
          </div>
            ...
            <div className="flex items-center gap-3 text-xs text-[var(--accent)] mt-2 pt-2 border-t border-[var(--border)]">
              {task.actualTimeMinutes !== null && (
                <span className="flex items-center gap-1">
                  ...
                  واقعی: {toPersianNum(task.actualTimeMinutes)} دقیقه
                </span>
              )}
              {task.actualTestCount !== null && (
                <span className="flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-[var(--accent)]" />
                  واقعی: {toPersianNum(task.actualTestCount)} تست
                </span>
              )}
            </div>
          )}
          ...
          {isPending ? (
            ...
            <div className="flex items-center gap-2">
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="text-[var(--accent)] text-xs font-bold flex items-center gap-1.5 bg-[var(--accent-soft)] px-2.5 py-1.5 rounded-md border border-[rgba(62,180,137,0.2)]"
              >
            ...
            <div className="flex items-center gap-2">
              <motion.span
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="text-[#F87171] text-xs font-bold flex items-center gap-1.5 bg-[rgba(239,68,68,0.12)] px-2.5 py-1.5 rounded-md border border-[rgba(239,68,68,0.2)]"
              >
