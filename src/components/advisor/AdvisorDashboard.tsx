...
function TaskModal({
  ...
  const [topic, setTopic] = useState(editTask?.topic ?? '');
  ...
  const handleSubmit = () => {
    ...
    const subjectColor = subjectObj?.color ?? '#3EB489';
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) resetForm(); onOpenChange(v); }}>
      <DialogContent className="bg-[var(--bg-overlay)] border-[var(--border-strong)] text-[var(--foreground)] max-h-[90vh] overflow-y-auto rounded-2xl" dir="rtl">
        ...
        <div className="space-y-4">
          <ModalSelect label="درس" value={subject} onChange={(e) => handleSubjectChange(e.target.value)}>
          ...
          <div>
            <label className="text-[11px] text-[var(--foreground-muted)] mb-1.5 block font-medium">نوع</label>
