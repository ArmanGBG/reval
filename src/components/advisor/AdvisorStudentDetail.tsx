...
export function AdvisorStudentDetail() {
  const { selectedStudentId, setCurrentView, setSelectedStudentId, tasks, exams, addTask, updateTask, deleteTask, addExam, advisorStudents, loadTasksForStudent, loadedStudentId, tasksLoading } = useAppStore();
  const students = advisorStudents;
  const analyses = useMemo(() => computeAnalyses(students), [students]);

  // Load this student's tasks from the API when the advisor opens the detail
  // (only if we haven't already loaded tasks for this student)
  useEffect(() => {
    if (selectedStudentId && selectedStudentId !== loadedStudentId) {
      loadTasksForStudent(selectedStudentId).catch(() => {});
    }
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="space-y-2 max-h-[28rem] overflow-y-auto custom-scrollbar pr-1 -mr-1">
                {studentTasks.map((task, index) => (
                  <motion.div
                    ...
                    onClick={() => !task.detailsCompleted && setDetailsTask(task)}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className="text-sm font-medium text-[var(--foreground)] truncate">{task.subject}</span>
                         {task.detailsCompleted ? <><span className="text-[var(--foreground-subtle)] text-xs">-</span><span className="text-xs text-[var(--foreground-muted)] truncate">{task.topic}</span></> : <span className="text-[10px] text-[var(--warning)] font-bold">جزئیات ناقص</span>}
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${
                         ...
                         {task.targetTimeMinutes != null && <span className="text-[10px] text-[var(--foreground-subtle)] flex items-center gap-0.5">
      ...
      />
      <TaskDetailsDialog task={detailsTask} open={!!detailsTask} onOpenChange={v => !v && setDetailsTask(null)} grade={student.grade} major={student.major} onSave={updates => updateTask(detailsTask!.id, updates)} />
