...
import { Copy, Link2, Loader2, UserCheck, UserPlus, X } from 'lucide-react';
...
export default function ConnectionManager({ role, onChanged }: { role: 'STUDENT' | 'ADVISOR'; onChanged?: () => void }) {
  ...
  return (
    <section className="surface-1 rounded-2xl p-4 md:p-5 space-y-4 border border-[var(--border)]">
      <div className="flex items-center gap-2"><Link2 className="w-5 h-5 text-mint" /><h3 className="font-bold">{role === 'STUDENT' ? 'مشاور من' : 'اتصال دانش‌آموزان'}</h3></div>
      ...
      {active && <button onClick={() => act(active.id, 'END')} className="text-xs text-red-400 border border-red-500/20 rounded-lg px-3 py-2">پایان همکاری</button>}
    </section>
  );
}
